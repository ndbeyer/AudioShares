...
const PortalProvider = ({ children }: { children: React.ReactNode }): React.Element => {
				...
				? contents.map((content, index) => {
						...
						return (
							<BackgroundWrapper key={`Portal${index}`}>
								...
								<ContentWrapper pad={isComponent ? '1.5rem' : '2.5rem'}>
									{isComponent ? (
										content
									) : (
										<>
											...
											<Row>
												{content?.buttons?.map(({ label, onPress, disabled, loading }) => (
													<Button
														margin="0 0 0 1rem"
